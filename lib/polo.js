...
/* --- elegir a quiénes, de entre los anotados -------------------------------
   Dos formas, las dos con el mismo contrato: reciben la lista de anotados EN
   ORDEN DE LLEGADA y devuelven `cantidad` de ellos, también en orden de
   llegada. Quién juega contra quién no se decide acá: eso lo sigue haciendo
   `repartirPorHandicap` con los elegidos.                                   */

/** Ordena como llegaron, sin depender de que la lista venga ordenada. */
const porLlegada = (jugadores) => {
  const orden = new Map(jugadores.map((j, i) => [j.id, i]));
  return (lista) => lista.slice().sort((a, b) => orden.get(a.id) - orden.get(b.id));
};

/**
 * **Por nivel**: los `cantidad` de más handicap interno. Si dos empatan, entra
 * el que se anotó primero — el reloj desempata, no el apellido.
 */
function elegirPorNivel(cantidad, anotados) {
  if (!esCantidadValida(cantidad)) {
    throw new ErrorDeArmado(`No existe el formato de ${cantidad} jugadores.`);
  }
  if (anotados.length < cantidad) {
    throw new ErrorDeArmado(`Hay ${anotados.length} anotados y la práctica es de ${cantidad}.`);
  }
  const enOrden = porLlegada(anotados);
  const posicion = new Map(anotados.map((j, i) => [j.id, i]));
  const elegidos = anotados.slice()
    .sort((a, b) => b.handicap - a.handicap || posicion.get(a.id) - posicion.get(b.id))
    .slice(0, cantidad);
  return enOrden(elegidos);
}

/**
 * **Parejo**: los `cantidad` con los que los equipos quedan lo más parejos
 * posible.
 *
 * Arranca por los primeros que se anotaron —que es el reparto justo— y de ahí
 * prueba cambios: saca a uno, mete a otro, y se queda con el cambio que más
 * achica la diferencia de handicap entre los equipos. Repite hasta que ningún
 * cambio mejora.
 *
 * El orden de llegada no es solo el punto de partida: también desempata. Entre
 * dos combinaciones igual de parejas se queda con la de los que se anotaron
 * antes, así el automático nunca saltea a alguien porque sí.
 */
function elegirParejo(cantidad, anotados) {
  if (!esCantidadValida(cantidad)) {
    throw new ErrorDeArmado(`No existe el formato de ${cantidad} jugadores.`);
  }
  if (anotados.length < cantidad) {
  ...
  const enOrden = porLlegada(anotados);
  const posicion = new Map(anotados.map((j, i) => [j.id, i]));
  // Cuánto "tarde" es este conjunto: menos es mejor, y desempata.
  const tardanza = (lista) => lista.reduce((a, j) => a + posicion.get(j.id), 0);
  const quePasa = (lista) => desbalance(repartirPorHandicap(cantidad, enOrden(lista)), cantidad);

  let elegidos = anotados.slice(0, cantidad);
  let mejor = quePasa(elegidos);
  let afuera = anotados.slice(cantidad);

  // Con el cupo justo no hay nada que probar, y el tope de vueltas es un freno
  // por si alguna vez dos cambios se turnan para mejorarse entre ellos.
  for (let vuelta = 0; afuera.length && vuelta < 40; vuelta += 1) {
    let cambio = null;

    elegidos.forEach((sale) => {
      afuera.forEach((entra) => {
        const prueba = elegidos.map((j) => (j.id === sale.id ? entra : j));
        const valor = quePasa(prueba);
        const gana = valor < mejor
          || (valor === mejor && cambio && tardanza(prueba) < tardanza(cambio.prueba));
        if (gana) cambio = { prueba, valor, sale, entra };
      });
    });

    if (!cambio || cambio.valor >= mejor) break;
    mejor = cambio.valor;
    afuera = afuera.map((j) => (j.id === cambio.entra.id ? cambio.sale : j));
    elegidos = cambio.prueba;
  }
...
function paraPantalla(planilla) {
  ...
  return {
    ...
    jugadores: jugadores.map((j) => ({
      ...
      nombre: j.nombre,
      apodo: j.apodo,
      // Solo sale con sentido si el que pidió la planilla puede ver el interno;
      // si no, `hcpConocido` es false y la pantalla no lo muestra.
      handicap: j.handicap,
      color: j.color,
      orden: j.orden,
      sale: j.sale,
...
/* --- la escalera de la flecha ---------------------------------------------
   La flecha no promedia una ventana de partidos: vive en cinco escalones y
   cada partido la sube o la baja. Arranca en el 4 —diagonal arriba— y no se
   pasa del 5 ni baja del 1.

   El tope es la regla, no un detalle de implementación: es lo que hace que la
   flecha diga *cómo viene* y no *cómo le fue todo el año*. Al que ganó veinte
   seguidos, un solo partido perdido ya lo baja un escalón, porque nunca juntó
   un colchón que después haya que descontar. Y de paso, como no puede estar a
   más de cuatro pasos de donde está, en los hechos solo se acuerda de los
   últimos cuatro o cinco partidos, sin tener que contar ventanas. */
const FLECHA_ARRANQUE = 4;
const FLECHA_TECHO = 5;
const FLECHA_PISO = 1;
const FLECHA_MEDIO = 3;          // el escalón horizontal
/** Ganar por MÁS de esto sube dos escalones en vez de uno. */
const GOLEADA_GANADA = 5;
/** Perder por MÁS de esto baja dos. Duele más tarde de lo que premia una
    goleada dada, y es a propósito: no es lo mismo darla que recibirla. */
const GOLEADA_PERDIDA = 7;
/** Salir MVP, además de lo que haya valido el partido. */
const PASOS_MVP = 2;
...
/**
 * Cuántos escalones mueve un partido. El empate es el único que no tiene un
 * valor fijo: acerca la flecha a la horizontal, así que depende de dónde esté.
 */
function pasosDelPartido(diferencia, escalon) {
  const d = Number(diferencia);
  if (d === 0) {
    // Un empate no es ni bueno ni malo: devuelve la flecha hacia el medio.
    if (escalon > FLECHA_MEDIO) return -1;
    if (escalon < FLECHA_MEDIO) return 1;
    return 0;
  }
  if (d > GOLEADA_GANADA) return 2;
  if (d > 0) return 1;
  if (d >= -GOLEADA_PERDIDA) return -1;
  return -2;
}
 ...
 *
 * Es una escalera de cinco escalones que arranca en el 4 —diagonal arriba— y
 * que cada partido sube o baja. Ganar sube uno, ganar por más de 5 sube dos,
 * perder baja uno, perder por más de 7 baja dos, el empate acerca un escalón a
 * la horizontal, y el MVP suma dos más aparte de lo que haya valido el partido.
 *
 * Después de cada partido se acota al techo y al piso. Acotar **en cada paso**
 * y no al final es lo que le da el carácter: nadie junta un colchón, así que
 * una sola derrota baja al que venía ganando todo.
 *
 * Devuelve el escalón corrido al cero: el 3 —la horizontal— vale 0.
 */
function flechaDe(resultados) {
  const yaPremiada = new Set();
  let escalon = FLECHA_ARRANQUE;

  resultados.forEach((r) => {
    escalon += pasosDelPartido(r.diferencia, escalon);
    // El MVP es de la práctica: en una de 12 el jugador tiene dos filas el
    // mismo día y el premio se cuenta una sola vez.
    if (r.mvp && !yaPremiada.has(r.practicaId)) {
      yaPremiada.add(r.practicaId);
      escalon += PASOS_MVP;
    }
    escalon = Math.max(FLECHA_PISO, Math.min(FLECHA_TECHO, escalon));
  });

  return escalon - FLECHA_MEDIO;
}
...
module.exports = {
  ...
  ajusteDeHandicap, flechaDe, comoViene, PALIZA, TOPE_AJUSTE,
  FLECHA_ARRANQUE, FLECHA_TECHO, FLECHA_PISO, GOLEADA_GANADA, GOLEADA_PERDIDA,
};
