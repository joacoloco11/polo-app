...
const { comoVienenTodos, temporadaActiva, hcpEfectivo } = require('../handicap');
const { anotadosParaArmar } = require('./anotaciones');
 ...
 */
async function traerJugadores(ids, yaEstaban = null) {
  ...
  // Al editar una práctica vieja puede haber alguien que desde entonces salió
  // del plantel. Ese jugó igual, así que se lo deja quedarse; lo que no se
  // puede es meter a alguien que ya no está.
  const vale = (id) => porId.has(id) && (porId.get(id).activo || (yaEstaban && yaEstaban.has(id)));
  const faltan = ids.filter((id) => !vale(id));
  if (faltan.length) {
    throw new ErrorDeArmado('Hay jugadores elegidos que ya no están en el plantel.');
  }

  // Para armar manda el handicap del club —no el de la AAP— y con el ajuste que
  // le fueron dando los resultados de la temporada puesto encima.
  ...
  return ids.map((id) => {
    ...
    return {
      ...
      nombre: j.nombre,
      apodo: j.apodo,
      handicap: hcpEfectivo(j.hcp_interno, como.get(j.id)),
    };
  });
}

/**
 * Quiénes de una convocatoria ya tienen práctica. De una lista de veinte salen
 * dos prácticas, y la segunda no puede volver a elegir a los de la primera.
 */
async function yaJuegan(convocatoriaId) {
  const filas = await consultar(
  ...
  return new Set(filas.map((f) => f.jugador_id));
}
...
async function armar(req, res, sesion) {
  ...
  const auto = ['nivel', 'parejo'].includes(String(datos.auto || '')) ? String(datos.auto) : null;
  ...
  if (auto) {
    // Armado automático: los elige el servidor de entre los anotados de ese
    // día. Nunca del plantel entero —el que no se anotó no juega— y nunca a
    // alguien que ya está en otra práctica de la misma convocatoria.
    const dia = await anotadosParaArmar(datos.fecha);
    if (!dia) {
      return error(res, 409, 'Ese día no tiene lista de anotados. Abrila en la solapa Anotación.');
    }
    const ocupados = await yaJuegan(dia.convocatoria.id);
    const libres = dia.anotados.filter((j) => !ocupados.has(j.id));
    if (libres.length < formato) {
      return error(res, 400, libres.length === dia.anotados.length
        ? `Hay ${libres.length} anotados y la práctica es de ${formato}.`
        : `Quedan ${libres.length} anotados sin práctica y esta es de ${formato}.`);
    }

    const elegidos = auto === 'nivel'
      ? elegirPorNivel(formato, libres)
      : elegirParejo(formato, libres);
    asignados = repartirPorHandicap(formato, elegidos);
  } else {
  ...
  // De qué convocatoria salió, si salió de alguna. Queda en null cuando el
  // admin arma a mano sin pasar por la lista de anotados, que sigue valiendo.
  const convocatoria = await unaFila(
    'select id from convocatoria where fecha = $1::date', [cabecera.fecha],
  ).catch(() => null);

  const guardada = await transaccion(async (tx) => {
    const practica = await tx.unaFila(
      `insert into practica
         (temporada_id, fecha, hora, cancha, tipo, formato, chukkers, estado, notas,
          creada_por, convocatoria_id)
       values ($1, $2, $3, $4, 'practica', $5, $6, 'publicada', nullif($7, ''), $8, $9)
      ...
      [temporada.id, cabecera.fecha, cabecera.hora, cabecera.cancha,
        formato, planilla.chukkers, notas, sesion.id, convocatoria ? convocatoria.id : null],
    );
...
// Editar una práctica ya publicada necesita lo mismo que armarla: la cabecera
// bien puesta y los jugadores con el handicap del club. Se comparte en vez de
// copiarse para que las dos pantallas validen igual.
module.exports.validarCabecera = validarCabecera;
module.exports.traerJugadores = traerJugadores;
module.exports.COLORES_VALIDOS = COLORES_VALIDOS;
