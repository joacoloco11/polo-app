...
module.exports = publico(async (req, res) => {
  ...
  // La llave maestra de la copia de prueba, antes que nada: abre cualquier
  // nombre —jugador o administrador— y entra como él, pero **no le toca el
  // PIN**. Si le pusiera uno, el primero que use la llave le dejaría la cuenta
  // cerrada al resto, que es justo lo que la llave viene a evitar. En la app del
  // club esto devuelve false siempre.
  if (esLlaveMaestra(pin)) {
    return ok(
      res,
      {
        jugador: {
      ...
      // La sesión queda atada a la llave: el día que cambies ese PIN, el que
      // entró con él se cae solo.
      { 'Set-Cookie': cookieDeSesion(jugador, true) },
    );
  }

  // Con la llave prendida, el alta de PIN queda apagada: en la copia de prueba
  // se entra con la llave y nada más.
  if (hayLlave()) {
