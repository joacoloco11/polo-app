...
async function nuevoPartido(req, res, sesion) {
  ...
  if (organizador === 'otro' && organizadorNombre.length < 2) {
    return error(res, 400, 'Poné quién organiza el torneo.');
  }

  const chukkers = enRango(datos.chukkers, 4, 8);
  if (chukkers === undefined) return error(res, 400, 'Los chukkers van de 4 a 8.');

  const hcpTorneo = enRango(datos.hcpTorneo, 0, 40);
  if (hcpTorneo === undefined) return error(res, 400, 'El handicap del torneo va de 0 a 40.');
  ...
  if (cancha === undefined) return error(res, 400, 'Elegí en qué cancha se jugó.');
  const sede = deLocal ? null : String(datos.sede || '').trim().slice(0, 80);
  if (!deLocal && sede.length < 2) return error(res, 400, 'Poné dónde se jugó.');
  ...
  if (aFavor === undefined || enContra === undefined) {
    return error(res, 400, 'Los goles no pueden ser negativos.');
  }
  if ((aFavor === null) !== (enContra === null)) {
    return error(res, 400, 'Cargá los dos marcadores o ninguno.');
  }
  ...
  const jornada = await unaFila(
    `insert into jornada (jugador_id, nombre, fecha, chukkers, medios,
     ...
     returning id, nombre, to_char(fecha, 'YYYY-MM-DD') as fecha, chukkers, medios,
               organizador, organizador_nombre, hcp_torneo, de_local, cancha, sede,
               goles_a_favor, goles_en_contra`,
    [sesion.id, nombre, datos.fecha, chukkers === null ? CHUKKERS_TORNEO : chukkers, medios,
      organizador, organizadorNombre, hcpTorneo, deLocal, cancha, sede, aFavor, enContra],
  );
  ok(res, { jornada });
}
