...
async function listar(res) {
  ...
  if (!temporada) return ok(res, { temporada: null, canchas: [], torneos: [] });

  const canchas = await consultar(
    `with usos as (
       select p.cancha, 'practica' as clase, count(pj.jugador_id)::int as jugadores
       from practica p
       left join practica_jugador pj on pj.practica_id = p.id
       where p.temporada_id = $1
       group by p.id, p.cancha
       union all
       select t.cancha, 'torneo' as clase, coalesce(t.jugadores, 0)::int
     ...
     select cancha,
            ...
            count(*)::int                                    as total,
            round(avg(jugadores) filter (where jugadores > 0), 1)::float8 as promedio_jugadores
     from usos
     group by cancha
     order by total desc, cancha`,
    [temporada.id],
  );

  const torneos = await consultar(
    `select t.id, t.nombre, t.tipo, to_char(t.fecha, 'YYYY-MM-DD') as fecha,
            to_char(t.hora, 'HH24:MI') as hora, t.cancha, t.jugadores
     from torneo t
  ...
  ok(res, { temporada, canchas, torneos });
}
...
async function sumarTorneo(req, res, sesion) {
  ...
  let jugadores = null;
  if (datos.jugadores !== undefined && datos.jugadores !== null && datos.jugadores !== '') {
    jugadores = Number(datos.jugadores);
    if (!Number.isInteger(jugadores) || jugadores < 0 || jugadores > 24) {
      return error(res, 400, 'Los jugadores van de 0 a 24.');
    }
  }
  ...
  const torneo = await unaFila(
    `insert into torneo (temporada_id, nombre, tipo, fecha, hora, cancha, jugadores, creado_por)
     values ($1, $2, $3, $4, $5, $6, $7, $8)
     returning id, nombre, tipo, to_char(fecha, 'YYYY-MM-DD') as fecha,
               to_char(hora, 'HH24:MI') as hora, cancha, jugadores`,
    [temporada.id, nombre, tipo, datos.fecha, hora, cancha, jugadores, sesion.id],
  );
  ok(res, { torneo });
}
