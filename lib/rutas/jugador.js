...
const { comoVieneUno, hcpEfectivo } = require('../handicap');
...
module.exports = conSesion(async (req, res, sesion) => {
  ...
  if (!temporada) {
    return ok(res, { jugador, temporada: null, practicas: [], resumen: null, como: null });
  }
  ...
  // Los partidos de torneo los carga el propio jugador, así que salen de sus
  // jornadas sin práctica. Van en la misma lista de abajo, mezclados por fecha.
  const torneos = await consultar(
    `select id, nombre, to_char(fecha, 'YYYY-MM-DD') as fecha, chukkers, medios,
            organizador, organizador_nombre as "organizadorNombre",
            hcp_torneo as "hcpTorneo", de_local as "deLocal", cancha, sede,
            goles_a_favor as "golesAFavor", goles_en_contra as "golesEnContra"
     from jornada
  ...
  const conResultado = torneos.filter((t) => t.golesAFavor !== null);
  const conHcp = torneos.filter((t) => t.hcpTorneo !== null);
  const resumenTorneos = {
    partidos: torneos.length,
    ganados: conResultado.filter((t) => t.golesAFavor > t.golesEnContra).length,
    empatados: conResultado.filter((t) => t.golesAFavor === t.golesEnContra).length,
    conResultado: conResultado.length,
    hcp: conHcp.length
      ? Math.round((conHcp.reduce((a, t) => a + t.hcpTorneo, 0) / conHcp.length) * 10) / 10
      : null,
  };
  ...
  // Cómo viene: la flecha la ve cualquiera, porque sale de resultados públicos.
  // El handicap interno y su ajuste, solo un administrador.
  const como = await comoVieneUno(temporada.id, id);
  if (sesion.admin) {
    const base = await unaFila('select hcp_interno from jugador where id = $1', [id]);
    como.hcpBase = base ? base.hcp_interno : 0;
    como.hcpEfectivo = hcpEfectivo(como.hcpBase, como);
  } else {
    delete como.ajuste;
  }
