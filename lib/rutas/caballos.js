...
const { companerosDe, caballadaDe } = require('../caballada');

const COLUMNAS = `id, nombre, activo, lesionado,
                  to_char(lesionado_desde, 'YYYY-MM-DD') as lesionado_desde`;

/**
 * El caballo que puedo tocar: el mío, o el de alguien que aceptó compartir la
 * caballada conmigo. Null si no es ninguno de los dos.
 *
 * Quien puede montarlo puede anotarle lo que jugó y marcarle una lesión: son
 * datos del animal, no del dueño. Lo que no se puede es darlo de baja —eso sí
 * es del dueño— y se frena aparte.
 */
async function caballoAlQuePuedo(caballoId, jugadorId) {
  const companeros = await companerosDe(jugadorId);
  return unaFila(
    `select id, nombre, jugador_id from caballo
      where id = $1 and jugador_id = any($2::uuid[])`,
    [caballoId, companeros],
  );
}
...
async function sumarExtra(datos, res, sesion) {
  const mio = await caballoAlQuePuedo(datos.caballo_id, sesion.id);
  if (!mio) return error(res, 404, 'Ese caballo no es de tu caballada.');
...
async function borrarExtra(req, res, sesion) {
  ...
  const fila = await unaFila(
    `delete from chukker_extra e
      using caballo c
      where e.id = $1 and c.id = e.caballo_id and c.jugador_id = any($2::uuid[])
      returning e.id`,
    [id, companeros],
  );
  if (!fila) return error(res, 404, 'Esa carga no es de tu caballada.');
...
async function guardar(req, res, sesion) {
  ...
  if (datos.id) {
    const mio = await caballoAlQuePuedo(datos.id, sesion.id);
    if (!mio) return error(res, 404, 'Ese caballo no es de tu caballada.');
    ...
    if (datos.activo !== undefined) {
      // Dar de baja un caballo sí es cosa del dueño: el del grupo lo monta,
      // no decide si sigue en la caballada.
      if (mio.jugador_id !== sesion.id) {
        return error(res, 403, mio.nombre + ' no es tuyo: lo da de baja su dueño.');
      }
